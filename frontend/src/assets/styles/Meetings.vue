<template>
    ...
    >
      ...
      <template v-slot:item.current_step="{ item }">
        ...
        >
          <template v-slot:activator="{ props }">
            <v-chip
              :color="item.custom_vocabulary && item.custom_vocabulary?.current_step !== CustomVocabularyProgressEnum.Published && item.current_step !== MeetingSetupProgressEnum.Sealed ? 'error' : getProgressColor(item.current_step!)"
              v-bind="props"
              text-color="white"
              small
            >
  ...
  <v-dialog persistent v-model="promptResponsesDialog" max-width="90%">
    <v-card>
      <v-card-title class="d-flex align-center mt-2">
      ...
      <v-card-text>
        ...
        >
          ...
          <template v-slot:item.prompt="{ item }">
            <div style="min-width: 20%">{{ item.prompt }}</div>
          </template>
          <template v-slot:item.prompt_response="{ item }">
  ...
  <v-dialog persistent v-model="meetingNotes" max-width="90%">
    <v-card style="max-height: 90vh; display: flex; flex-direction: column">
      ...
      <b-row class="d-flex pr-10" style="width: 100%; justify-content: right">
        <b-col class="mb-12">
          <div class="confidence-chart-container">
            <div class="confidence-chart">
              <div 
                ...
                {{ confidenceStats.veryHigh > 5 ? confidenceStats.veryHigh.toFixed(1) + '%' : '' }}
                <span class="confidence-segment-tooltip">Very High (98-100%): {{ confidenceStats.veryHigh.toFixed(1) }}%</span>
              </div>
              <div 
                   class="confidence-segment high"
                   :style="{width: (confidenceStats.high || 0.1) + '%'}">
                {{ confidenceStats.high > 5 ? confidenceStats.high.toFixed(1) + '%' : '' }}
                <span class="confidence-segment-tooltip">High (90-98%): {{ confidenceStats.high.toFixed(1) }}%</span>
              </div>
              <div 
                   class="confidence-segment medium"
                   :style="{width: (confidenceStats.medium || 0.1) + '%'}">
                {{ confidenceStats.medium > 5 ? confidenceStats.medium.toFixed(1) + '%' : '' }}
                <span class="confidence-segment-tooltip">Medium (60-90%): {{ confidenceStats.medium.toFixed(1) }}%</span>
              </div>
              <div 
                   class="confidence-segment low"
                   :style="{width: (confidenceStats.low || 0.1) + '%'}">
                {{ confidenceStats.low > 5 ? confidenceStats.low.toFixed(1) + '%' : '' }}
                <span class="confidence-segment-tooltip">Low (40-60%): {{ confidenceStats.low.toFixed(1) }}%</span>
              </div>
              <div 
                   class="confidence-segment very-low"
                   :style="{width: (confidenceStats.veryLow || 0.1) + '%'}">
                {{ confidenceStats.veryLow > 5 ? confidenceStats.veryLow.toFixed(1) + '%' : '' }}
                <span class="confidence-segment-tooltip">Very Low (0-20%): {{ confidenceStats.veryLow.toFixed(1) }}%</span>
              </div>
            </div>
            <div class="confidence-legend">
              <div class="legend-item">
                <div class="legend-color very-high"></div>
                <span>Very High (98-100%)</span>
              </div>
              <div class="legend-item">
                <div class="legend-color high"></div>
                <span>High (90-98%)</span>
              </div>
              <div class="legend-item">
                <div class="legend-color medium"></div>
                <span>Medium (60-90%)</span>
              </div>
              <div class="legend-item">
      ...
      <div class="d-flex" style="flex: 1; overflow: hidden">
        <div style="width: 70%; border-right: 1px solid rgba(0, 0, 0, 0.12)">
          <v-card-text
            v-if="isDownloadingMeetingJson"
          ...
          <v-card-text
            v-if="!isDownloadingMeetingJson"
            class="transcript-container flex-grow-1"
            style="overflow-y: auto; padding: 20px"
            @keydown="handleKeyDown"
          >
            <template v-if="meetingNotesObject?.segments.length">
              <div
                v-for="segment in meetingNotesObject.segments"
                :key="segment.segment_id"
                :class="{ segment: true, 'new-speaker': true }"
              >
                <div
                  class="speaker-label clickable"
                  @click="
                    openSpeakerDialog(
                ...
                <div class="segment-content continued-speech">
                  <span
                    v-for="word in filterWords(segment)"
                    :class="{
                      selectedWord: isWordSelected(word.word_id),
                      'low-confidence': isLowConfidence(word),
                      editedWord: word.edited,
                      'known-phrase': existingVocabularyPhrases.some(
                        (phrase) =>
                          phrase.phrase?.toLowerCase() ===
                            word.content.trim().toLowerCase() ||
                          (phrase.display_as &&
                            phrase.display_as.toLowerCase() ===
                              word.content.trim().toLowerCase())
                      ),
                    ...
                    @mousedown.prevent
                    @click="(event) => showConfidence(word, word.word_id, event)"
                  >
                    {{ word.content.trim() }}
                  </span>
                </div>
        ...
        <div style="width: 30%; overflow-y: auto; padding: 20px">
          ...
          <div class="changes-table">
            <h3 class="text-h6 mb-2">Changes History</h3>
            <v-table density="compact" hover>
              ...
              <tbody>
                <tr
                  v-for="change in meetingNotesObject?.changes"
                  :key="change.timestamp"
                  :class="{
                    'highlight-change': selectedWords.some(
                      (w) => w.word_id === change.word_id
                    ),
                  }"
                >
      ...
      >
        <div class="d-flex flex-wrap w-100" v-if="!isDownloadingMeetingJson">
  ...
  <v-dialog persistent v-model="customVocabularyDialog" max-width="500px">
    <v-card>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="error" variant="text" @click="closeCustomVocabularyDialog"
          >Cancel</v-btn
        >
        <v-btn color="primary" @click="saveToCustomVocabulary">Add</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
...
export default {
  ...
  setup() {
              ...
              .map((segment: SpeakerSegmentType) => {
                ...
                if (segmentWords) {
                  ...
                  segment.transcript = segmentWords
                    .map((word: SpeakerWordType) => word.content)
                    .join(' ');
    ...
    const isDownloadingMeetingJson = ref(false);
    ...
    const closeSpeakerDialog = () => {
      ...
      editedSpeaker.name = ''
      editedSpeaker.startTime = 0
      editedSpeaker.endTime = 0
      editedSpeaker.speaker_label = ''
    }

    const updateSpeakerName = () => {
      if (!editedSpeaker.speaker_label || !editedSpeaker.name) {
        return
      }
      ...
      try {
        ...
        // Update processed transcript
        processedTranscript.value.forEach((segment: SpeakerSegmentType) => {
          if (segment.speaker_label === editedSpeaker.speaker_label) {
            segment.speaker_label = editedSpeaker.name
          }
        })
    ...
    const openSpeakerDialog = (speakerLabel: string | undefined, startTime: number | undefined, endTime: number | undefined) => {
      editedSpeaker.speaker_label = speakerLabel
      editedSpeaker.name = speakerLabel ?? ''
      editedSpeaker.startTime = startTime ?? 0
      editedSpeaker.endTime = endTime ?? 0
    ...
    const processedTranscript = ref<SpeakerSegmentType[]>([]);
    ...
    const clearMeetingNotesState = () => {
      ...
      isDownloadingMeetingJson.value = false;
    ...
    const openMeetingNotesDialog = async (item: MeetingResponse) => {
      ...
      try {
        ...
        downloadProgressMeetingObject.value = 0;
        isDownloadingMeetingJson.value = true;
    ...
    const isWordSelected = (wordIndex: number) => {
      return selectedWordIds.value.includes(wordIndex);
    };
        ...
        if (response?.segments) {
          ...
          response.segments.forEach((segment: any) => {
            if (segment.speaker_label) {
              ...
              // Get words for this segment's time range
              const segmentWords = response.words
                .filter((item: SpeakerWordType) => item.start_time &&
    ...
    const fetchVocabularyPhrases = async (vocabularyId: string) => {
      try {
        ...
        // Update UI to show known phrases
        if (processedTranscript.value) {
          processedTranscript.value = [...processedTranscript.value];
        }
      } catch (error) {
    ...
    const filterWords = (segment: SpeakerSegmentType): SpeakerWordType[] => {
      if (segment === undefined)
        console.log("segment_id is undefined");
      const words = meetingNotesObject.value?.words.filter((word: SpeakerWordType) => word.segment_id == segment.segment_id) || [];
      return words;
    };
    ...
    const exportAnalyticsToPDF = async () => {
      try {
        ...
        const element = document.querySelector('.analytics-card');
        if (!element) {
        ...
        const charts = element.querySelectorAll('.apexcharts-canvas');
        const chartPromises = Array.from(charts).map(chart => {
          return new Promise((resolve) => {
            // Convert SVG to canvas
            const svgData = chart.querySelector('svg');
            if (!svgData) {
              resolve(null);
              return;
            }
        ...
        // Title
        doc.setFontSize(20);
        doc.text('Meeting Analytics Report', margin, 40);
        ...
        // Meeting Overview
        doc.setFontSize(16);
        doc.text('Meeting Overview', margin, yPos);
        yPos += 25;

        doc.setFontSize(12);
        doc.text(`Title: ${meetingAnalytics.value?.meetingMetadata?.meetingTitle}`, margin, yPos);
        yPos += lineHeight;
        doc.text(`Date: ${new Date(meetingAnalytics.value?.meetingMetadata?.dateTime || '').toLocaleString()}`, margin, yPos);
        yPos += lineHeight;
        doc.text(`Duration: ${Math.round((meetingAnalytics.value?.meetingMetadata?.meetingDuration || 0) / 60)} minutes`, margin, yPos);
        yPos += lineHeight;
        doc.text(`Type: ${meetingAnalytics.value?.meetingMetadata?.meetingType}`, margin, yPos);
        yPos += lineHeight;
        doc.text(`Format: ${meetingAnalytics.value?.meetingMetadata?.virtualOrInPersonOrHybrid}`, margin, yPos);
        yPos += 30;
        ...
        doc.setFontSize(12);
        const summaryText = doc.splitTextToSize(`${meetingAnalytics.value?.meetingSummary.overview.summary}`, pageWidth);
        doc.text(summaryText, margin, yPos);
        yPos += (summaryText.length * lineHeight) + 20;
          ...
          chartImages.forEach((imgData, index) => {
            if (imgData) {
              ...
              if (typeof imgData === 'string') {
                doc.addImage(imgData, 'PNG', margin, yPos, pageWidth, 200);
              }
              yPos += 220;
            }
          });
        ...
        if (meetingAnalytics.value?.actionItems?.items?.length) {
          ...
          doc.setFontSize(12);
          meetingAnalytics.value.actionItems.items.forEach(item => {
            if (yPos > doc.internal.pageSize.getHeight() - 50) {
              doc.addPage();
              yPos = 40;
            }

            doc.text(`• ${item.actionItemDescription}`, margin, yPos);
            yPos += lineHeight;
            doc.text(`  Assignee: ${item.actionItemAssignee}`, margin + 10, yPos);
            yPos += lineHeight;
            doc.text(`  Due Date: ${new Date(item.actionItemDueDate).toLocaleDateString()}`, margin + 10, yPos);
            yPos += lineHeight;
            doc.text(`  Priority: ${item.actionItemPriority}`, margin + 10, yPos);
            yPos += lineHeight;
            doc.text(`  Status: ${item.actionItemStatus}`, margin + 10, yPos);
            yPos += 20;
          });
        }

        // Save PDF
        const fileName = `meeting-analytics-${meetingAnalytics.value?.meetingMetadata?.meetingTitle || 'report'}.pdf`;
        doc.save(fileName);
        showSuccess('Analytics exported to PDF successfully');
      } catch (error) {
        console.error('Error exporting analytics to PDF:', error);
        showError('Failed to export analytics to PDF');
      } finally {
